...
namespace FluentWait.Tests.UnitTesting
{
    /// <summary>
    /// </summary>
    /// <seealso cref="FluentWait.Contracts.IScriptHandler" />
    [TestClass]
    public class FluentWaitHandlerAssertionTests : IScriptHandler
    {
        public IWaitHandler waitHandler { get; set; }

        [TestInitialize]
        public void TestInitialize()
        {
            waitHandler = new WaitHandler().SetPollingInterval(TimeSpan.FromSeconds(1)).SetTimeout(TimeSpan.FromSeconds(60));
        }
        ...
        [TestMethod]
        [Aspects.WaitHandlerResetAspect(2, 1)]
        public void AssertCollectionIsNotNullAndIsAnyAndIsEqual()
        {
            IWaitBuilder builder = new WaitBuilder(waitHandler);
